...
def install_req():
    if platform.system() == "Windows":
        subprocess.call("py -m pip install -r requirements.txt", shell=True)
    else:
        if sys.version_info[0] < 3:
            subprocess.call("py -m pip install -r requirements.txt", shell=True)
        else:
            subprocess.call("py -m pip3 install -r requirements.txt", shell=True)
...
labol = tkinter.Label(
    root, text="Owner's username (to prevent other users to use it) without @:"
)
